...
const Loan = {
    ...
    async create(loanData) {
        const { userId, loanType, amount, purpose, tenureMonths, interestRate, monthlyEMI, totalAmount } = loanData;
        
        const [result] = await pool.execute(
            `INSERT INTO loans (user_id, loan_type, amount, purpose, tenure_months, interest_rate, monthly_emi, total_repayment) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [userId, loanType, amount, purpose, tenureMonths, interestRate, monthlyEMI, totalAmount]
        );
