...
const User = {
  ...
  async create(userData) {
    ...
    try {
      ...
      console.log(
        "[User Model] User created successfully, ID:",
        result.insertId
      );
  ...
  async findByEmail(email) {
    ...
    console.log("[User Model] User found:", user ? "Yes" : "No");
    return user;
  },
  ...
  async findById(id, includePassword = false) {
    ...
    const user = await db.getAsync(`SELECT ${fields} FROM users WHERE id = ?`, [
      id,
    ]);
    return user;
  },
  ...
  async update(id, updateData) {
    ...
    try {
      ...
      console.log(
        "[User Model] Update successful, affected rows:",
        result.changes
      );
