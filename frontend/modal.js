        ...
        modalOverlay.innerHTML = `
            <div class="modal-content modal-${size}">
                ...
                <div class="modal-body">
                    <p>${message}</p>
                    ${type === 'prompt' || type === 'form' ? this.createInputs(inputs) : ''}
                </div>
                <div class="modal-footer">
                    ${type !== 'alert' ? `<button class="btn btn-outline modal-cancel">${cancelText}</button>` : ''}
                    <button class="btn btn-primary modal-confirm">${type === 'alert' ? 'OK' : confirmText}</button>
                </div>
            </div>
        `;
...
//Globally available modal format..
window.Modal = Modal;